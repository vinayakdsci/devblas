...
template <typename T>
void
benchmark_gemm (const char *name, int warmup_iters, int iters, GemmFn<T> fn,
                devblas_gemm_config_t *config)
{
    ...
    uniform_distribution_selector<T> dist (static_cast<T> (-5),
                                           static_cast<T> (5));
...
template void benchmark_gemm<int> (const char *name, int warmup_iters,
                                   int iters, GemmFn<int>,
                                   devblas_gemm_config_t *config);
