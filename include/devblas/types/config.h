...
struct GemmConfig
{
        ...
        GemmConfig (Layout layout, int M, int N, int K, int lda, int ldb,
                    int ldc, int tileSize)
            : layout_ (layout), M_ (M), N_ (N), K_ (K), lda_ (lda), ldb_ (ldb),
              ldc_ (ldc), tileSize_ (tileSize)
        {
        }
        ...
        GemmConfig (Layout layout, int M, int N, int K, int lda, int ldb,
                    int ldc)
            : layout_ (layout), M_ (M), N_ (N), K_ (K), lda_ (lda), ldb_ (ldb),
              ldc_ (ldc), tileSize_ (std::nullopt)
        {
        }
...
inline GemmConfig
gemm_config_to_cpp (devblas_gemm_config_t *config)
{
    if (!config)
        {
    ...
    if (config->tileSize < 0)
        {
            return GemmConfig (layout_to_cpp (config->layout), config->M,
                               config->N, config->K, config->lda, config->ldb,
                               config->ldc);
        }
